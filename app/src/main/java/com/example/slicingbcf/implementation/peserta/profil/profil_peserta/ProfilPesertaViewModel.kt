package com.example.slicingbcf.implementation.peserta.profil.profil_peserta

import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.example.slicingbcf.data.common.UiState
import com.example.slicingbcf.data.local.ProfilPeserta
import com.example.slicingbcf.data.local.profilPeserta
import com.example.slicingbcf.di.IODispatcher
import com.example.slicingbcf.di.MainDispatcher
import dagger.hilt.android.lifecycle.HiltViewModel
import kotlinx.coroutines.CoroutineDispatcher
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.asStateFlow
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext
import javax.inject.Inject


@HiltViewModel
class ProfilPesertaViewModel @Inject constructor(
  @IODispatcher private val ioDispatcher : CoroutineDispatcher,
  @MainDispatcher private val mainDispatcher : CoroutineDispatcher
) : ViewModel() {

  private val _state = MutableStateFlow<UiState<ProfilPeserta>>(UiState.Loading)
  val state = _state.asStateFlow()

  init {
    loadDetailData()
  }

  fun onEvent(event : ProfilPesertaEvent) {
    when (event) {
      is ProfilPesertaEvent.ReloadData -> {
        loadDetailData()
      }
    }
  }

  private fun loadDetailData() {

    viewModelScope.launch(ioDispatcher) {
      try {
//        delay(1500)
        val data = profilPeserta[0]
        withContext(mainDispatcher) {
          _state.value = UiState.Success(data)
        }
      } catch (e : Exception) {
        withContext(mainDispatcher) {
          _state.value = UiState.Error(e.message ?: "Terjadi kesalahan")
        }
      }
    }
  }
}

sealed class ProfilPesertaEvent {
  object ReloadData : ProfilPesertaEvent()
}
